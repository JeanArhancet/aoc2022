# Solution found on reddit (for the part2)
    ...
    for _ in range(rounds):
        for i, monkey in enumerate(monkeys):
            while monkey_items[i]:
                ...
                target_monkey = (
                    monkey["true"] if value % monkey["test"] == 0 else monkey["false"]
                )
                monkey_items[target_monkey].append(
                    value if worry_divisor != 1 else value % modulo
                )
                inspected[i] += 1
    inspected.sort()
